#!/usr/bin/env bash
# maduin install script — wire plugin into Emacs (Doom-aware).
# Usage:
#   ./harness/install.sh            install
#   ./harness/install.sh --uninstall  remove wiring (keeps repo)
# Re-run anytime (idempotent). Symlinks → repo edits live via M-x maduin-reload.

set -euo pipefail

HARNESS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SNIPPET_MARKER=";; === maduin (managed by harness/install.sh) ==="
...
uninstall() {
  if [ -f "${TARGET}" ]; then
    ...
    awk -v start="${SNIPPET_MARKER}" -v end=";; === end maduin ===" '
      $0 == start {skip=1}
